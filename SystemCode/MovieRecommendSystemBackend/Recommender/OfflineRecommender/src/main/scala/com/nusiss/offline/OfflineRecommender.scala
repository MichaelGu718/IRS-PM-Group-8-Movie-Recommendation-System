...
case class MovieRating(userId: Int, movieId: Int, rating: String, timestamp: String )
...
object OfflineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ratingRDD = spark.read
      ...
      .as[MovieRating]
      .rdd
      .map( rating => ( rating.userId, rating.movieId, rating.rating ) )    // 转化成rdd，并且去掉时间戳
      .cache()
    ...
    // 训练隐语义模型
    val trainData = ratingRDD.map( x => Rating(x._1, x._2, x._3.toDouble) )
