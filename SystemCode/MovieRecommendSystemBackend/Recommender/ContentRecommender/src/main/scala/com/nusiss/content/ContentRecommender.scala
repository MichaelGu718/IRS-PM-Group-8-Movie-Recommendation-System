...
case class Movie(movieId: Int, title: String, plotSummary: String, runtime: String, releaseYear: String,
                 languages: String, genres: Array[String], actors: Array[String], directors: Array[String])
  ...
  def main(args: Array[String]): Unit = {
    ...
    val movieTagsDF = spark.read
      ...
      .map(
        ...
        x => ( x.movieId, x.title,
          (x.genres++x.directors).mkString("|").map(c=> if(c=='|') ' ' else c) )
      )
      .toDF("movieId", "title", "mixFeature")
      .cache()

    // 核心部分： 用TF-IDF从内容信息中提取电影特征向量

    // 创建一个分词器，默认按空格分词
    val tokenizer = new Tokenizer().setInputCol("mixFeature").setOutputCol("words")
    ...
    val movieRecs = movieFeatures.cartesian(movieFeatures)
      ...
      .groupByKey()
      .map{
        case (movieId, items) => MovieRecs( movieId, items.toList.sortWith(_._2 > _._2).map(x => Recommendation(x._1, x._2)) )
      }
      .toDF()
    movieRecs.write
      .option("uri", mongoConfig.uri)
